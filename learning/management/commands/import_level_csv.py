...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        lang, _ = Language.objects.get_or_create(
            code=opts["code"], defaults={"name": opts["language"].title()}
        )
        course, _ = Course.objects.get_or_create(
            language=lang, slug=opts["course"], defaults={"title": f"{lang.name} – A1"}
        )
        level, _ = Level.objects.get_or_create(
            course=course, number=opts["level"], defaults={"title": f"Niveau {opts['level']}"}
        )
        ...
        for k, slug in TOPIC_MAP.items():
            topics[k] = Topic.objects.get_or_create(
                course=course, slug=slug, defaults={"name": k.title()}
            )[0]
